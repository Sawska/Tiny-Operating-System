...
void ip_init(uint8_t* ip_addr) {
    memcpy(local_ip, ip_addr, IP_ADDR_LEN);
}
...
void ip_send_icmp_echo_reply(const uint8_t* request_packet, uint16_t len) {
    ...
    memcpy(reply_buf, request_packet, total_len);

    ip_header_t* rep_ip = (ip_header_t*)reply_buf;

    memcpy(rep_ip->src, req_ip->dst, IP_ADDR_LEN);
    memcpy(rep_ip->dst, req_ip->src, IP_ADDR_LEN);
...
void ip_input(const uint8_t* data, uint16_t len) {
    ...
    if (memcmp(ip_hdr->dst, local_ip, IP_ADDR_LEN) != 0) {
        return;
    }
